...
psnr_vec_16 =  df_16.iloc[:, 9]
psnr_vec_restormer =  df_32.iloc[:, 9]
psnr_vec_corrupted =  df_corrupted.iloc[:, 9]
psnr_vec_mcnet =  df_mcnet.iloc[:, 9]


print(stats.ttest_rel(psnr_vec_mcnet, psnr_vec_16))
print(stats.ttest_rel(psnr_vec_restormer, psnr_vec_16))
print(stats.ttest_rel(psnr_vec_corrupted, psnr_vec_16))




dataset = pd.concat([psnr_vec_corrupted, psnr_vec_mcnet, psnr_vec_restormer, psnr_vec_16], axis=1)
...
x1, x2 = 3, 4
y, h, col = dataset.max().max() , 2, 'k'
ax.plot([x1, x1, x2, x2], [y, y+h, y+h, y], lw=1.5, c=col)
ax.text((x1+x2)*.5, y+h, "p<0.005", ha='center', va='bottom', color=col)
x1, x2 = 2, 4
y, h, col = dataset.max().max() , 5, 'k'
ax.plot([x1, x1, x2, x2], [y, y+h, y+h, y], lw=1.5, c=col)
ax.text((x1+x2)*.5, y+h, "p<0.005", ha='center', va='bottom', color=col)
x1, x2 = 1, 4
y, h, col = dataset.max().max() , 8, 'k'
ax.plot([x1, x1, x2, x2], [y, y+h, y+h, y], lw=1.5, c=col)
ax.text((x1+x2)*.5, y+h, "p<0.005", ha='center', va='bottom', color=col)
ax.grid(True)
...
plt.ylabel('PSNR')
plt.savefig('violinplot_psnr.png', dpi=600)
